...
public final class NeoTransaction {
    ...
    /// The NeoRpcClient instance of this transaction.
    public var rpcClient: NeoRpcClient?
    ...
    public init(rpcClient: NeoRpcClient? = nil, version: Byte, nonce: Int, validUntilBlock: Int, signers: [Signer], systemFee: Int, networkFee: Int, attributes: [TransactionAttribute], script: Bytes, witnesses: [Witness], blockCountWhenSent: Int? = nil) {
    ...
    public func send() async throws -> NeoSendRawTransaction {
        ...
        let rpcClient = try requireRpcClient()
        blockCountWhenSent = try await rpcClient.getBlockCount().send().getResult()
        return try await rpcClient.sendRawTransaction(hex).send()
    }
    ...
    public func track() throws -> AnyPublisher<Int, Error> {
        guard let blockCountWhenSent = blockCountWhenSent else {
            throw NeoError.illegalState("Cannot subscribe before transaction has been sent.")
        }
        let rpcClient = try requireRpcClient()
        ...
        return rpcClient.catchUpToLatestAndSubscribeToNewBlocksPublisher(blockCountWhenSent, true)
    ...
    public func getApplicationLog() async throws -> NeoApplicationLog {
        guard blockCountWhenSent != nil else {
            throw NeoError.illegalState("Cannot get the application log before transaction has been sent.")
        }
        let rpcClient = try requireRpcClient()
        return try await rpcClient.getApplicationLog(getTxId()).send().getResult()
    }
    
    public func requireRpcClient() throws -> NeoRpcClient {
        guard let rpcClient = rpcClient else {
            throw NeoError.illegalState("NeoTransaction has not been assigned NeoRpcClient instance.")
        }
        return rpcClient
    }
...
extension NeoTransaction: NeoSerializable {
    ...
    public func getHashData() async throws -> Bytes {
        let rpcClient = try requireRpcClient()
        return try await rpcClient.getNetworkMagicNumberBytes() + toArrayWithoutWitnesses().sha256()
    }
    ...
    public func toContractParametersContext() async throws -> ContractParametersContext {
        ...
        let rpcClient = try requireRpcClient()
        let network = try await rpcClient.getNetworkMagicNumber()
        ...
        let items: [String: ContractParametersContext.ContextItem] = try signers.map { signer in
            guard let accountSigner = signer as? AccountSigner else {
                throw NeoError.unsupportedOperation("Cannot handle contract signers")
            }
            guard let verificationScript = accountSigner.account.verificationScript else {
                throw NeoError.illegalState("Account on AccountSigner has no verification script")
            }
