...
public final class VerificationScript: NeoSerializable, Hashable {
    ...
    public init(_ publicKeys: [ECPublicKey], _ signingThreshold: Int) throws {
        guard signingThreshold >= 1 && signingThreshold <= publicKeys.count else {
            throw NeoError.illegalArgument("Signing threshold must be at least 1 and not higher than the number of public keys.")
        }
        guard publicKeys.count <= NeoConstants.MAX_PUBLIC_KEYS_PER_MULTISIG_ACCOUNT else {
            throw NeoError.illegalArgument("At max \(NeoConstants.MAX_PUBLIC_KEYS_PER_MULTISIG_ACCOUNT) public keys can take part in a multi-sig account")
        }
