...
public extension ECPrivateKey {
    ...
    convenience init(_ key: BInt) throws {
        ...
        guard keyString.count <= NeoConstants.PRIVATE_KEY_SIZE * 2 else {
            throw NeoError.illegalArgument("Private key must fit into \(NeoConstants.PRIVATE_KEY_SIZE) bytes, but required \(keyString.count / 2) bytes.")
        }
    ...
    convenience init(_ key: Bytes) throws {
        guard key.count == NeoConstants.PRIVATE_KEY_SIZE else {
            throw NeoError.illegalArgument("Private key byte array must have length of \(NeoConstants.PRIVATE_KEY_SIZE) but had length \(key.count)")
        }
